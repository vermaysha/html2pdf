...
export async function convertToPdf(options: ConversionOptions): Promise<void> {
  const { executablePath, inputSource, outputFile, timeout, pageFormat, removeSource } = options;
  ...
  try {
    ...
    console.log(`PDF generated with size: ${(buffer.length / 1024).toFixed(2)} KB`);
    ...
    console.error('❌ An error occurred during the PDF conversion process:', error);
    process.exit(1);
  } finally {
